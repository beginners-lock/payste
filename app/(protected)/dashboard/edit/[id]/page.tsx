...
import { createUserPayste, getPaysteById, updateUserPayste } from "@/service/payste.service"
import { useParams, useRouter } from "next/navigation"
import { PAYSTE_PAGE } from "@/utils/routes"
import { Spinner } from "@/components/ui/spinner"
import { useEffect, useState } from "react"
import { usePayste } from "@/hooks/usePayste"
import EditPaysteLoading from "./loading"
import EditPaysteError from "./error"
...
export default function CreatePastePage({}) {
  ...
  const { payste, pending, isError, error } = usePayste(params.id as string)
  ...
  const onSubmit = handleSubmit(async ({ title, content }) => {
    if(!payste) { toast.error('Could not resolve payste ID'); return; }

    const response = await updateUserPayste(payste!.id, title, content)
  ...
  if(pending || !payste){
    return <EditPaysteLoading />
  }

  if(isError && error){
    return <EditPaysteError error={error} reset={()=>{ window.location.reload() }}/>
  }
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            <Button type="submit" className="bg-primary hover:bg-primary/90" disabled={isSubmitting}>
              {isSubmitting ? <Spinner/> : "Update Paste"}
            </Button>
